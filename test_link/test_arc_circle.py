...
def test_circle_by_center_fail():
    ctx = Mgx3D.getStdContext()
    ctx.clearSession() # Clean the session after the previous test
    gm = ctx.getGeomManager ()
    p0 = gm.newVertex (Mgx3D.Point(0, 0, 0))
    ...
    p3 = gm.newVertex (Mgx3D.Point(0, 0.001, 0))

    # the first call should fail because of an infinite curvature
    gm.newArcCircle("Pt0000", "Pt0001", "Pt0002", True)
    assert gm.getNbCurves()==0
    gm.newArcCircle("Pt0003", "Pt0001", "Pt0002", True)
    assert gm.getNbCurves()==1
