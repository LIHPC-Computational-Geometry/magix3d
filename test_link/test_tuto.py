...
cylindre_or_mli_file_name = "cylindreOreille.mli2"
...
def test_tuto_cylindreOreille():
    ctx = Mgx3D.getStdContext()
    ctx.clearSession() # Clean the session after the previous test
    exec(open(tuto_scripts_dir + "/cylindreOreille.py").read())
    gm = ctx.getGeomManager()
    assert gm.getNbVolumes()==3
    mm = ctx.getMeshManager()
    assert mm.getNbVolumes()==2
    assert os.path.exists(cylindre_or_mli_file_name)
    assert os.path.getsize(cylindre_or_mli_file_name) > 0
    assert mm.getNbRegions()==88080 #Nb cells