...
def unit_test(radius, length):
    ...
    tm.newCylinderWithTopo (Mgx3D.Point(0, 0, 0), radius, Mgx3D.Vector(length, 0, 0), 360, True, .5, 10, 10, 10)
    # Ar0013 : arête de la face latérale
    # le bloc a pour longueur d'arete la diagonale du carre de cote radius
    assert math.isclose(tm.getEdgeLength("Ar0013"), square_diagonal(radius), abs_tol=1e-15)
    # la longueur projetee est un quart de circonference
    assert math.isclose(tm.getEdgeProjectedLength("Ar0013"), circumference_quarter(radius), abs_tol=1e-15)
    # Ar0023 : arête de du côté du cylindre
    # le bloc a pour longueur la longur du cylindre
    assert math.isclose(tm.getEdgeLength("Ar0023"), length, abs_tol=1e-15)
    # la longueur projetee est la longueur du bloc
    assert math.isclose(tm.getEdgeProjectedLength("Ar0023"), length, abs_tol=1e-15)
    ...
    assert math.isclose(tm.getEdgeLength("Ar0014"), square_diagonal(radius) / 2.0, abs_tol=1e-15)
    # l'arete est associée à une face coplanaire avec le plan XY, donc la longueur projetée est égale à la longueur d'arete
    assert math.isclose(tm.getEdgeProjectedLength("Ar0014"), square_diagonal(radius) / 2.0, abs_tol=1e-15)
    # Ar0046 : arête de côté du o-grid
    # le bloc a pour longueur la longur du cylindre
    assert math.isclose(tm.getEdgeLength("Ar0046"), length, abs_tol=1e-15)
    # l'arête n'est associée à aucune entité géométrique, donc la longueur projetée est égale à la longueur d'arete
    assert math.isclose(tm.getEdgeProjectedLength("Ar0046"), length, abs_tol=1e-15)
