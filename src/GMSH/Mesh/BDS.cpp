...
{
    //std::cout<<"BDS_Mesh::recover_edge("<<num1<<", "<<num2<<")"<<std::endl;
    BDS_Edge *e = find_edge(num1, num2);
    _fatal = false;

    if(e) {
        //std::cout<<" find_edge a réussi"<<std::endl;
        return e;
    }
    ...
    printf("edge %d %d has to be recovered\n", num1, num2);
    std::cout<<" p1 XYZ: "<<p1->X<<", "<<p1->Y<<", "<<p1->Z<<",  UV: "<<p1->u<<", "<<p1->v<<std::endl;
    std::cout<<" p2 XYZ: "<<p2->X<<", "<<p2->Y<<", "<<p2->Z<<",  UV: "<<p2->u<<", "<<p2->v<<std::endl;
            ...
            if(!e->deleted && e->p1 != p1 && e->p1 != p2 && e->p2 != p1 && e->p2 != p2)
                if(Intersect_Edges_2d(e->p1->u, e->p1->v,
                    ...
                    if(e2r && e2r->find(EdgeToRecover(e->p1->iD, e->p2->iD, 0)) != e2r->end()){
                        ...
                        printf("edge %d %d on model edge %d cannot be recovered because"
                                " it intersects %d %d on model edge %d\n", num1, num2, itr2->ge->tag(),
                                e->p1->iD, e->p2->iD, itr1->ge->tag());

                        BDS_Point *pp1 = find_point(e->p1->iD);
                        BDS_Point *pp2 = find_point(e->p2->iD);

                        std::cout<<" pp1 XYZ: "<<pp1->X<<", "<<pp1->Y<<", "<<pp1->Z<<",  UV: "<<pp1->u<<", "<<pp1->v<<std::endl;
                        std::cout<<" pp2 XYZ: "<<pp2->X<<", "<<pp2->Y<<", "<<pp2->Z<<",  UV: "<<pp2->u<<", "<<pp2->v<<std::endl;
...
//                outputScalarField(triangles, "debugr.pos", 0);
                ...
                printf("edge %d %d cannot be recovered at all, look at debugp.pos "
                        "and debugr.pos\n", num1, num2);
                _fatal = true;
                return 0;
            }
