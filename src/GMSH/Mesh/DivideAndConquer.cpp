...
{
    Msg::Info("DocRecord::Qtest avec h : %d, i : %d, j : %d, k : %d",h, i, j, k);
  if((h == i) && (h == j) && (h == k)) {
...
//  std::cout<<"Point h : "<<pa[0]<<", "<<pa[1]<<std::endl;
//  std::cout<<"Point i : "<<pb[0]<<", "<<pb[1]<<std::endl;
//  std::cout<<"Point j : "<<pc[0]<<", "<<pc[1]<<std::endl;
//  std::cout<<"Point k : "<<pd[0]<<", "<<pd[1]<<std::endl;

  // we use robust predicates here
  double result = robustPredicates::incircle(pa, pb, pc, pd) *
    robustPredicates::orient2d(pa, pb, pc);

//  std::cout<<((result < 0)?"point k is NOT in the circumcircle of triangle  hij"
//          :"point k is in the circumcircle of triangle  hij")<<std::endl;
