                                ...
                                const SPoint3 &aPoint)
{
  ...
  if (method == GModelFactory::THREE_POINTS){
	  ...
	  Handle(Geom_Curve) crv = GC_MakeArcOfCircle (aP1, aP2, aP3);
#else
	  Handle(Geom_Curve) crv = GC_MakeArcOfCircle (aP1, aP2, aP3).Value().get();
#endif
    if (occv1 && occv2)
      occEdge = BRepBuilderAPI_MakeEdge(crv,occv1->getShape(),
                                        occv2->getShape()).Edge();
    else
      occEdge = BRepBuilderAPI_MakeEdge(crv).Edge();
  }
...
#if (OCC_VERSION_MAJOR == 6)
    Handle(Geom_Curve) crv = new Geom_TrimmedCurve(C, Alpha1, Alpha2, false);
#else
    Handle(Geom_Curve) crv1 = C.get();
    Handle(Geom_TrimmedCurve) arc = new Geom_TrimmedCurve(crv1, Alpha1, Alpha2, false);
    Handle(Geom_Curve) crv = arc.get();
#endif
    if (occv1 && occv2)
      occEdge = BRepBuilderAPI_MakeEdge(crv, occv1->getShape(),
                                        occv2->getShape()).Edge();
			     ...
			     std::vector<std::vector<double> > points)
{
  ...
  if (type == BEZIER) {
#if (OCC_VERSION_MAJOR == 6)
	  Handle(Geom_Curve) crv = new Geom_BezierCurve(ctrlPoints);
#else
	  Handle(Geom_BezierCurve) Bez = new Geom_BezierCurve(ctrlPoints);
	  Handle(Geom_Curve) crv =  Bez.get();
#endif

    if (occv1 && occv2)
      occEdge = BRepBuilderAPI_MakeEdge(crv,occv1->getShape(),occv2->getShape()).Edge();
    else
      occEdge = BRepBuilderAPI_MakeEdge(crv).Edge();
  }
  return gm->_occ_internals->addEdgeToModel(gm, occEdge);
			    ...
			    std::vector<int> mult)
{
  ...
  TopoDS_Edge occEdge;
#if (OCC_VERSION_MAJOR == 6)
  Handle(Geom_Curve) crv = NURBS;
#else
  Handle(Geom_Curve) crv = NURBS.get();
#endif
  if (occv1 && occv2)
    occEdge = BRepBuilderAPI_MakeEdge(crv, occv1->getShape(),
                                      occv2->getShape()).Edge();
