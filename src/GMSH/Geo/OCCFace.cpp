...
void OCCFace::setup()
{
    //std::cout<<"OCCFace::setup()"<<std::endl;
    edgeLoops.clear();
    l_edges.clear();
    l_dirs.clear();
...
#ifdef _DEBUG_MESH
  printf("OCC Face %d with %d parameter bounds (%g,%g)(%g,%g)\n",
             tag(), l_edges.size(), umin, umax, vmin, vmax);
#endif
...
#else
  occface = BRep_Tool::Surface(s).get();
#endif
...
#else
	  gp_Pln pl;// = Handle(Geom_Plane)::DownCast(occface)->Pln();
	  throw std::exception(); // OCCFace::containsPoint non porté en OCC7
#endif
...
#else
		throw std::exception(); // OCCFace::getSurfaceParams non porté en OCC7
		//p.radius = 0.0;
#endif
		break;
	case GEntity::Sphere:
...
bool OCCFace::buildSTLTriangulation(bool force)
{
  ...
  //withShare = Standard_False;
  params.InternalVerticesMode = Standard_False; //inshape ?
  params.Relative = Standard_True;
  //shapetrigu = Standard_False;
  BRepMesh_FastDiscret aMesher(aBox, params);
#endif
	...
	for (; aExpW.More(); aExpW.Next()) {
		...
		for (; aExpE.More(); aExpE.Next()) {
			...
			if (!BRep_Tool::Degenerated(aER)) {
				...
				// orient image
				// F. LEDOUX pour passage OCC 6.9.0
//				Standard_Boolean bIsToReverse =
//						BOPTools_AlgoTools3D::IsSplitToReverse1(aER, aE, myContext);
//				if (bIsToReverse) {
//					aER.Reverse();
//				}
			}
...
#else
        gp_Ax3 pos;
    	throw std::exception(); // OCCFace::isSphere non porté en OCC7
#endif
