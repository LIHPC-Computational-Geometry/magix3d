...
#else
	curve = BRep_Tool::Curve(c, s0, s1).get();
#endif
...
#else
    curve2d = BRep_Tool::CurveOnSurface(c, *s, s0, s1).get();
#endif
...
#else
        c2d = BRep_Tool::CurveOnSurface(c, *s, t0, t1).get();
#endif
    }
    else{
#if (OCC_VERSION_MAJOR == 6)
...
#else
        c2d = BRep_Tool::CurveOnSurface(c_rev, *s, t0, t1).get();
#endif
    }
//    std::cout<<"OCCEdge::reparamOnFace  BRep_Tool::CurveOnSurface => t0 = "<<t0<<", t1 = "<<t1
//            <<" , GEdge::tag : "<<tag()<<std::endl;
...
#else
			throw std::exception(); // OCCEdge::writeGEO non porté en OCC7
#endif
		}
		else{
#if (OCC_VERSION_MAJOR == 6)
