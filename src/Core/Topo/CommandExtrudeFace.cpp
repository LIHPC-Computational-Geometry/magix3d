...
/*----------------------------------------------------------------------------*/
        ...
        internalExecute()
        {
            ...
            message << "CommandExtrudeFace::execute pour la commande " << getName ( )
            ...
            for(int i_f = 0; i_f < new_cofaces.size(); i_f++){

                //------------------------
                //Méthode brutale, à voir une méthode plus intelligente
                //------------------------

                CoFace* f = new_cofaces[i_f];
                //On réinitialise toutes les associations géométriques pour les faces dupliquées
                ...
                for (Group::Group2D* gr : m_cofaces[i_f]->getGroupsContainer()){
                    std::cout<<gr->getName()<<std::endl;
                    gr->add(f);
                    f->getGroupsContainer().push_back(gr);
                }
            }
...
/*----------------------------------------------------------------------------*/
        void CommandExtrudeFace::constructExtrudeCoEdges(std::vector<CoFace*>& cofaces_0,
...
/*----------------------------------------------------------------------------*/
        void CommandExtrudeFace::constructExtrudeFaces(std::vector<CoFace*>& cofaces_0,
                                                       std::vector<CoFace*>& cofaces_1,
                                                       std::map<Vertex*, std::vector<CoEdge* > >& vtx2coedges,
                                                       std::map<CoEdge*, std::vector<CoFace* > >& coedge2cofaces)
        {
...
/*----------------------------------------------------------------------------*/
        void CommandExtrudeFace::constructExtrudeBlocks(std::vector<CoFace*>& cofaces_0,
                                                        std::vector<CoFace*>& cofaces_1,
                                                        std::map<Vertex*, std::vector<CoEdge* > >& vtx2coedges,
                                                        std::map<CoEdge*, std::vector<CoFace* > >& coedge2cofaces,
                                                        uint ni)
        ...
        std::cout<<std::endl;
                ...
                for (uint i=0; i<coface_0->getNbEdges(); i++){
                    ...
                    for (CoEdge* coedge_0 : edge_0->getCoEdges()){
                        std::vector<CoFace* >& revol_cofaces = coedge2cofaces[coedge_0];
                        for (CoFace* revol_coface : revol_cofaces)
                            face_cofaces.push_back(revol_coface);
                    }
#ifdef _DEBUG2
        	...
        	std::cout<<std::endl;
                    ...
                    if (!face_cofaces.empty()){
                        ...
                        for (CoEdge* coedge_0 : edge_0->getCoEdges()){
                            uint ratio = edge_0->getRatio(coedge_0);
                            if (ratio != 1){
#ifdef _DEBUG2
                                ...
                                std::vector<CoFace* >& revol_cofaces = coedge2cofaces[coedge_0];
                                for (CoFace* revol_coface : revol_cofaces)
                                    faces.back()->setRatio(revol_coface, ratio, 1);
                            }
                        }
...
//        		computeFaceRatio(faces.back(), 0, ni_loc_coface);
                ...
                Group::Group3D *group = getContext().getGroupManager().getNewGroup3D(getContext().getGroupManager().getDefaultName(3),
                                                                                          &getInfoCommand());
                group->add(newBlock);
                newBlock->getGroupsContainer().push_back(group);
...
/*----------------------------------------------------------------------------*/
        void CommandExtrudeFace::getPreviewRepresentation(Utils::DisplayRepresentation& dr)
        {
