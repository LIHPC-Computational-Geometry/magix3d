...
, m_save_mesh_data(0)
{
    ...
    m_topo_property->getFaces().insert(m_topo_property->getFaces().end(), faces.begin(), faces.end());
    m_topo_property->getVertices().insert(m_topo_property->getVertices().end(), vertices.begin(), vertices.end());
...
, m_save_mesh_data(0)
{
    ...
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(0,0,0)));
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(1,0,0)));
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(0,1,0)));
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(1,1,0)));
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(0,0,1)));
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(1,0,1)));
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(0,1,1)));
    m_topo_property->getVertices().push_back(new Topo::Vertex(ctx, Utils::Math::Point(1,1,1)));
    ...
    m_topo_property->getFaces().push_back(new Topo::Face(ctx, coface));
...
/*----------------------------------------------------------------------------*/
Utils::EntitySet<CoFace*> Block::getCoFaces() const
{
    Utils::EntitySet<CoFace*> l_cf(&Utils::Entity::compareEntity);
    for (Face* face : getFaces())
...
/*----------------------------------------------------------------------------*/
Utils::EntitySet<Edge*> Block::getEdges() const
{
    Utils::EntitySet<Edge*> l_e(&Utils::Entity::compareEntity);
    for(Face* face : getFaces())
        for(CoFace* coface : face->getCoFaces())
...
/*----------------------------------------------------------------------------*/
Utils::EntitySet<Vertex*> Block::getAllVertices() const
{
    Utils::EntitySet<Vertex*> l_v(&Utils::Entity::compareEntity);
    for (Face* face : getFaces())
        for (CoFace* coface : face->getCoFaces())
            for (Edge* edge : coface->getEdges())
...
/*----------------------------------------------------------------------------*/
Utils::EntitySet<CoEdge*> Block::getCoEdges() const
{
    Utils::EntitySet<CoEdge*> l_e(&Utils::Entity::compareEntity);
    for (Face* face : getFaces())
        for (CoFace* coface : face->getCoFaces())
            for (Edge* edge : coface->getEdges())
                for (CoEdge* coedge : edge->getCoEdges())
...
replace(Topo::Vertex* v1, Topo::Vertex* v2, bool propagate_up, bool propagate_down, Internal::InfoCommand* icmd)
{
    ...
    if (propagate_down)
    	for (Face* face : getFaces())
    		face->replace(v1, v2, propagate_up, propagate_down, icmd);
}
/*----------------------------------------------------------------------------*/
void Block::
...
getRepresentation(Utils::DisplayRepresentation& dr, bool checkDestroyed) const
{
    ...
    if (tdr->hasRepresentation(Utils::DisplayRepresentation::WIRE)){
        ...
        if (getVertices().size() == 8 && isStructured()){

            for (Vertex* vertex : getVertices())
                points.push_back(barycentre + (vertex->getCoord() - barycentre) * shrink);
        ...
        if (getVertices().size() == 8 && isStructured()){
            ...
            for (Face* face : getFaces()){
                ...
                tr.setShrink(1.0);
                face->getRepresentation (tr, checkDestroyed);
    ...
    std::cout<<"    points_coedge3[0] = "<<points_coedge3[0]<<", points_coedge3[1] = "<<points_coedge3[1]
    		...
    		for (CoEdge* coedge : getCoEdges())
    			if (coedge->find(getVertex(0)))
    				coedges_som0.push_back(coedge);
...
getDescription (bool alsoComputed) const
{
    ...
    Utils::SerializedRepresentation  vertices ("Sommets topologiques",
            TkUtil::NumericConversions::toStr(getVertices().size()));
    for (Topo::Vertex* v : getVertices())
    ...
    if (Internal::InternalPreferences::instance ( )._displayFace.getValue ( )){
    	Utils::SerializedRepresentation faces ("Faces topologiques",
    			TkUtil::NumericConversions::toStr(getFaces().size()));

    	for (Topo::Face* face : getFaces())
    		faces.addProperty (
    				Utils::SerializedRepresentation::Property (
    						face->getName ( ), *face));
...
//    log (TkUtil::TraceLog (message1, TkUtil::Log::TRACE_4));
    ...
    m_topo_property->getFaces().clear();
    m_topo_property->getFaces().insert(m_topo_property->getFaces().end(), sorted_faces.begin(), sorted_faces.end());
...
check() const
{
    ...
    if (isStructured()){
        ...
        for (uint iFace=0; iFace<getFaces().size(); iFace++){
...
/*----------------------------------------------------------------------------*/
std::vector<Vertex*> Block::getHexaVertices() const
{
    ...
    } else if (getVertices().size() == 5){
        for (uint i=0; i<5; i++)
            sommets[i] = getVertex(i);
        sommets[5] = sommets[4];
        sommets[6] = sommets[4];
        sommets[7] = sommets[4];
    } else if (getVertices().size() == 6){
        for (uint i=0; i<4; i++)
            sommets[i] = getVertex(i);

        if (getFace(0)->getVertices().size() == 3 && getFace(1)->getVertices().size() == 3){
            sommets[4] = getVertex(4);
            sommets[5] = getVertex(5);
            sommets[6] = getVertex(4);
            sommets[7] = getVertex(5);
        } else if (getFace(2)->getVertices().size() == 3 && getFace(3)->getVertices().size() == 3){
            sommets[4] = getVertex(4);
...
//            <<v3->getName()<<")"<<std::endl;
    ...
    for (Face* face : getFaces()) {
        uint nb_marques = 0;
        const std::vector<Topo::Vertex* > & local_vertices = face->getVertices();
...
TkUtil::UTF8String & operator << (TkUtil::UTF8String & o, const Block & b)
{
    ...
    o << " avec "<<(short)b.getFaces().size()<<" faces:\n";
    for (Face* face : b.getFaces())
        o << " "<<face->getName()<<"\n";
//      o << *faces[i]<<"\n";
    o << "\n";

    o << b.getName()<< " bloc avec "<<(short)b.getVertices().size()<<" sommets:\n";
    const std::vector<Topo::Vertex* > & vertices = b.getVertices();
    for (Vertex* vertex : b.getVertices()){
        o <<"   "<<*vertex<<"\n";
    }
...
Topo::TopoInfo Block::getInfos() const
{
    ...
    infos.incident_coedges = Utils::toVector(getCoEdges());
	infos.incident_cofaces = Utils::toVector(getCoFaces());
...
{
    std::cout << "setProperty " << getName() << " : " << getMeshLawName() << "-->" << prop->getMeshLawName() << std::endl;
...
//    std::cout<<"Block::saveInternals pour "<<getName()<<std::endl;
    ...
    if (m_save_topo_property) {
//        std::cout<<"m_save_topo_property avec "<<m_save_topo_property->getVertices().size()<<" sommets"<<std::endl;
        ...
        Internal::InfoCommand* icmd)
{
    ...
    for (uint i=0; i<faces.size() && !faceATrouvee; i++){
        const std::vector<Vertex*>& vertices = faces[i]->getVertices();
...
//        std::cout <<"Block::fuse ne trouve pas de face A\n";
    ...
    const std::vector<Face*>& bloc_B_faces = bloc_B->getFaces();
    for (uint i=0; i<bloc_B_faces.size() && !faceBTrouvee; i++){
        const std::vector<Topo::Vertex* > vertices = bloc_B_faces[i]->getVertices();
        bool tousMarques = true;
        for (uint j=0; j<vertices.size() && tousMarques; j++)
        ...
        Internal::InfoCommand* icmd)
{
   ...
   TkUtil::Timer timer(true);
    ...
    Utils::EntitySet<CoFace*> blk_cofaces = getCoFaces();
   ...
   // le découpage des faces communes (qui doivent former une boucle)
    ...
    Face* face1 = (getVertices().size()!=8 && dirSplit==k_dir?0:getFace(dirSplit*2+1));
    Face* faceI[4];
    Face* newFaceI[4][2];
#ifdef _DEBUG_SPLIT
    ...
    for (uint i=0; i<nbFacesCoupees; i++){
        ...
        if (tabIndFaceParDir[dirSplit][i]<getFaces().size()){
    ...
    for (uint i=0; i<nbFacesCoupees; i++){
        ...
        Topo::Vertex* vtx1;
        Topo::Vertex* vtx2;
        if (getVertices().size() == 6 && ind_face0 == 0 && i == 3)
            vtx1 = getVertex(5);
        else
            vtx1 = getVertex(TopoHelper::tabIndVtxByFaceOnBlock[ind_face0][i]);

        if (getVertices().size() == 6 && nbFacesCoupees == 4 && ind_face0 == 0 && i == 2)
            vtx2 = getVertex(5);
        else
    ...
    for (Group::Group3D* gr : getGroups()){
    	gr->add(block_1);
    	gr->add(block_2);
    	block_1->getGroups().push_back(gr);
    	block_2->getGroups().push_back(gr);
    }
            ...
            Internal::InfoCommand* icmd)
{
    ...
    for (uint f=0; f<4; f++)
        if (tabIndFaceParDir[dir][f]<getFaces().size()){
...
degenerateFaceInVertex(uint id, Internal::InfoCommand* icmd)
{
    ...
    coface->free(icmd);
    face->free(icmd);
    Utils::remove(m_topo_property->getFaces(), face, true);

    // suppression des sommets de la liste
    m_topo_property->getVertices().resize(5);
            ...
            Internal::InfoCommand* icmd)
{
	...
	if (getVertex(4) == getVertex(5))
		m_topo_property->getVertices()[5] = getVertex(6);
	m_topo_property->getVertices().resize(6);
...
//#define _DEBUG_PERM
    ...
    for (std::vector<CoEdge* >::iterator iter1 = coedges.begin();
        ...
        for (Edge* edge : (*iter1)->getEdges())
            filtre_aretes[edge] = 3;
    }
    ...
    for (Topo::Vertex* v : getFace(id)->getVertices())
        filtre_sommets[v] = 2;
    ...
    m_topo_property->getVertices()[0] = somOpp[0];
    m_topo_property->getVertices()[1] = somOpp[1];
    m_topo_property->getVertices()[2] = somOpp[2];
    m_topo_property->getVertices()[3] = p3;
    m_topo_property->getVertices()[4] = p4;
    m_topo_property->getVertices()[5] = p5;
    m_topo_property->getVertices()[6] = p6;
    m_topo_property->getVertices()[7] = p7;
...
//#define _DEBUG_REVERSE
	...
	for (Face* face : getFaces())
		if (3 == face->getNbVertices())
			faces_tri.push_back(face);
	...
	uint ind_deg = getIndexOf(vtx_deg);
	...
	Vertex* vtx_opp1 = faces_quad[0]->getVertex((faces_quad[0]->getIndexOf(vtx_deg)+2) %4);
	Vertex* vtx_opp2 = faces_quad[1]->getVertex((faces_quad[1]->getIndexOf(vtx_deg)+2) %4);
	uint ind_opp1 = getIndexOf(vtx_opp1);
	uint ind_opp2 = getIndexOf(vtx_opp2);
	...
	getBlockTopoProperty()->getVertices().clear();
	getBlockTopoProperty()->getVertices().insert(getBlockTopoProperty()->getVertices().end(), loc_vtx.begin(), loc_vtx.end());

	// idem avec les faces
	getBlockTopoProperty()->getFaces().clear();
	getBlockTopoProperty()->getFaces().insert(getBlockTopoProperty()->getFaces().end(), loc_faces.begin(), loc_faces.end());
...
void Block::getGroupsName(std::vector<std::string>& gn, bool byGeom, bool byTopo) const
{
	...
	if (byTopo)
		for (Group::Group3D* grp : m_topo_property->getGroups())
		    gn.push_back(grp->getName());
}
/*----------------------------------------------------------------------------*/
...
unsigned long Block::getNbInternalMeshingNodes()
{
	if (isStructured()){
		...
		for (Vertex* vtx : getVertices())
			res -= vtx->getNbInternalMeshingNodes();

		for (CoEdge* coedge : getCoEdges())
			res -= coedge->getNbInternalMeshingNodes();

		for (CoFace* coface : getCoFaces())
			res -= coface->getNbInternalMeshingNodes();
