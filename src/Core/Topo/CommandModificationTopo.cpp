    ...
    for (std::list<Geom::GeomEntity*>::iterator iter = geom_crv.begin();
        ...
        if ((*iter)->isDestroyed()){
            ...
            if (!newEntities.empty()) {
                ...
                if (newEntities.size() == 1){
                    Geom::Curve* curve0 = dynamic_cast<Geom::Curve*>(newEntities[0]);
                	auto geom_vertices = curve0->getVertices();
                ...
                for (uint i=0; i<newEntities.size(); i++){
                    Geom::Curve* curve = dynamic_cast<Geom::Curve*>(newEntities[i]);
                	auto geom_vertices = curve->getVertices();
...
#endif
            for (Vertex* vtx : coedges[j]->getVertices())
                if (vtx->getGeomAssociation() == oldGE){
                    getInfoCommand().addTopoInfoEntity(vtx, Internal::InfoCommand::DISPMODIFIED);
                    vtx->saveTopoProperty();
                    vtx->setGeomAssociation(newGE);
                }
...
//                    std::cout << " blocks[1]->getGeomAssociation():"<<(blocks[1]->getGeomAssociation()?blocks[1]->getGeomAssociation()->getName():"vide")<<std::endl;
    ...
    for (uint i=0; i<v_blocks.size(); i++){
        for (Face* face : v_blocks[i]->getFaces()) {
            std::vector<CoFace*> cofaces = face->getCoFaces();
            if (cofaces.size() != 1){
                TkUtil::UTF8String	message (TkUtil::Charset::UTF_8);
                message << "Une face ("<<face->getName()
    ...
    for (Topo::CoFace* coface : v_cofaces_to_delete){
        for (Face* f : coface->getFaces())
            f->free(&getInfoCommand());
...
//        	std::vector<Geom::GeomEntity*> newEntities = m_command_edit_geom->getReplacedEntities()[ge];
        	...
        	if (newEntities.size() >= 2){
        		...
        		for (uint k=0; k<newEntities.size(); k++){
            		auto curves = dynamic_cast<Geom::Surface*>(newEntities[k])->getCurves();
...
getSameCoEdges(Topo::CoEdge* coedge)
{
    ...
    for (Topo::CoEdge* vtx0_ce : vertices[0]->getCoEdges()){
        vertices = vtx0_ce->getVertices();
        uint nbVertexMarques = 0;
        for (Topo::Vertex* vtx : vertices)
            nbVertexMarques += filtre_vertices[vtx];

        if (nbVertexMarques == vertices.size() && vtx0_ce != coedge)
            coedges_trouvees.push_back(vtx0_ce);
    }
    		...
    		std::vector<Topo::Block*>& blocks)
{
    ...
    for (uint k=0; k<geoms.size(); k++){
    	auto surfaces = dynamic_cast<Geom::Volume*>(geoms[k])->getSurfaces();
