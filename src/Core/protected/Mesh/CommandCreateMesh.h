...
public:
    ...
    std::vector<Cloud*>& createdClouds () {return m_created_clouds;}

    /// Accesseur sur les noms des surfaces de maillage créés par la commande
    std::vector<Surface*>& createdSurfaces () {return  m_created_surfaces;}

    /// Accesseur sur les noms des volumes de maillage créés par la commande
    std::vector<Volume*>& createdVolumes () {return  m_created_volumes;}
