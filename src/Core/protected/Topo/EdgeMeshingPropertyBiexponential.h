...
public:
     ...
     */
    EdgeMeshingPropertyBiexponential(int nb, double r1, double sp1, double r2, double sp2, bool isDirect=true);
...
private:
    ...
    double computeParamExpo(double size, double length, uint nbrEdges) const;

    ///
    double computeIntermediateParameter(double paramExpo, double refParameter) const;

    ///
    std::vector<double> computeCoeffs(double paramExpo, double length, uint nbrEdges);
    void findNbrEdgesBalance(uint &n1_min, uint &n2_min);
