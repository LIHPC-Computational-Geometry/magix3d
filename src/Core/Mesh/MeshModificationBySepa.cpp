/*----------------------------------------------------------------------------*/
/*
 * \file MeshModificationBySepa.cpp
 *
 *  \author legoff
 *
 *  \date 8 jan. 2015
...
MeshModificationBySepa::writeControlVTKFile(const std::string& AFilename)
{
	...
	if(m_sepafiles.size() == 0) {
		...
		message <<"MeshModificationBySepa::applyModification Pas de fichier de contrôle pour la séparatrice.";
...
//#endif
//	writeDeltaVTKFile("/tmp/legoff/after_sepa");
#else
	throw TkUtil::Exception (TkUtil::UTF8String ("Package Sepa3D non inclu", TkUtil::Charset::UTF_8));
#endif
}
