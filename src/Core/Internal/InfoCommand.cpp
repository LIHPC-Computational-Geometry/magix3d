...
/*----------------------------------------------------------------------------*/
bool operator == (const InfoCommand::GeomEntityInfo& gel, const InfoCommand::GeomEntityInfo& ger)	// CP operator == for issue#103 workaround
{
	return (gel.m_geom_entity == ger.m_geom_entity) && (gel.m_type == ger.m_type) ? true : false;
}
void InfoCommand::addGeomInfoEntity(Geom::GeomEntity* entity, type t)
{
    ...
    // CP Issue#103 workaround : ensure unicity
    if (m_geom_entities_info.end ( ) != std::find (m_geom_entities_info.begin ( ), m_geom_entities_info.end ( ), gei))
	return;
