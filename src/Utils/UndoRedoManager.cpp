...
/*----------------------------------------------------------------------------*/
void UndoRedoManager::store(Command* command)
{
...
void UndoRedoManager::clearUndone()
{
    ...
    std::vector<Command*>    oldCommands = m_undone;
    m_undone.clear ( );
    for (std::vector<Command*>::iterator iter = oldCommands.begin();
...
/*----------------------------------------------------------------------------*/
const std::vector<Command*>& UndoRedoManager::getDoneCommands ( ) const
{
	return m_done;
}	// UndoRedoManager::getDoneCommands
/*----------------------------------------------------------------------------*/
const std::vector<Command*>& UndoRedoManager::getUndoneCommands ( ) const
{
	return m_undone;
}	// UndoRedoManager::getUndoneCommands
/*----------------------------------------------------------------------------*/
Command* UndoRedoManager::undoableCommand ( )
{
	AutoMutex	autoMutex (mutex ( ));

	Command*	cmd	= 0 == m_done.size( ) ? 0 : m_done [m_done.size( ) - 1];
...
std::string UndoRedoManager::undoableCommandName ( ) const
{
	...
	const Command*	cmd	= 0 == m_done.size( ) ? 0 : m_done [m_done.size( ) - 1];
...
/*----------------------------------------------------------------------------*/
Command* UndoRedoManager::redoableCommand ( )
{
	AutoMutex	autoMutex (mutex ( ));

	Command*	cmd	= 0 == m_undone.size ( ) ?
...
std::string UndoRedoManager::redoableCommandName ( ) const
{
	...
	const Command*	cmd	= 0 == m_undone.size ( ) ?
						  0 : m_undone [m_undone.size ( ) - 1];
...
/*----------------------------------------------------------------------------*/
bool UndoRedoManager::hasCommand (Command* command)
{
	AutoMutex	autoMutex (mutex ( ));
