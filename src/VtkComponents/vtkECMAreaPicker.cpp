...
vtkStandardNewMacro(vtkECMAreaPicker);

//--------------------------------------------------------------------------
vtkECMAreaPicker::vtkECMAreaPicker()
{
  this->FrustumExtractor = vtkECMExtractSelectedFrustum::New();	// Magix3D
...
void vtkECMAreaPicker::Initialize()
{
...
//--------------------------------------------------------------------------
void vtkECMAreaPicker::SetRenderer(vtkRenderer *renderer)
{
  this->Renderer = renderer;
}
//--------------------------------------------------------------------------
void vtkECMAreaPicker::SelectCompletelyInside (bool on)	// Magix3D
{
	vtkECMExtractSelectedFrustum*	extractor	= dynamic_cast<vtkECMExtractSelectedFrustum*>(FrustumExtractor);
	if (0 == extractor)
		return;

	extractor->SelectCompletelyInside (on);
}	// vtkECMAreaPicker::SelectCompletelyInside
//--------------------------------------------------------------------------
bool vtkECMAreaPicker::GetSelectCompletelyInside ( ) const	// Magix3D
{
	...
	return false;
}	// vtkECMAreaPicker::GetSelectCompletelyInside
//--------------------------------------------------------------------------
void vtkECMAreaPicker::SetPickCoords(double x0, double y0, double x1, double y1)
{
  this->X0 = x0;
  this->Y0 = y0;
  this->X1 = x1;
...
int vtkECMAreaPicker::AreaPick(double x0, double y0, double x1, double y1,
...
void vtkECMAreaPicker::DefineFrustum(double x0, double y0, double x1, double y1,
                                  vtkRenderer *renderer)
{
  this->X0 = (x0 < x1) ? x0 : x1;
  this->Y0 = (y0 < y1) ? y0 : y1;
...
int vtkECMAreaPicker::PickProps(vtkRenderer *renderer)
{
  vtkProp *prop;
  int pickable;
  double bounds[6];
...
int vtkECMAreaPicker::TypeDecipher(vtkProp *propCandidate,
...
int vtkECMAreaPicker::ABoxFrustumIsect(double *bounds, double &mindist)
{
...
//--------------------------------------------------------------------------
void vtkECMAreaPicker::PrintSelf(ostream& os, vtkIndent indent)
{
